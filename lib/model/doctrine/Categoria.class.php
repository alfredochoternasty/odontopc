...
class Categoria extends BaseCategoria
{
	...
	public function getCantVendida($p_zona=0) {
		$sql = "
			...
			from 
				listado_ventas l
					join grupoprod g on l.grupoprod_id = g.id
			where 
				g.categoria_id = ".$this->id."
				and l.tipofactura_id <> 4
				and l.fecha >= date_format(curdate(), '%Y-%m-01')
				and ($p_zona = 0 or l.zona_id = $p_zona)
		";
	...
	public function getCantVendidaAnt($p_zona=0) {
		$sql = "
			...
			where 
				g.categoria_id = ".$this->id."
				and l.tipofactura_id <> 4
				and l.fecha between date_format(date_sub(curdate(), interval 1 month), '%Y-%m-01') and date_sub(curdate(), interval 1 month)
				and ($p_zona = 0 or l.zona_id = $p_zona)
		";
		$vendidos = $this->ejecutarSQL($sql);
	...
	public function getCantVendidaHist($p_zona=0) {
		$sql = "
			...
			where 
				g.categoria_id = ".$this->id."
				and l.tipofactura_id <> 4
				and l.fecha between date_format(date_sub(curdate(), interval 13 month), '%Y-%m-01') and last_day(date_sub(curdate(), interval 1 month))
				and ($p_zona = 0 or l.zona_id = $p_zona)
			group by
				anio, mes
			order by
