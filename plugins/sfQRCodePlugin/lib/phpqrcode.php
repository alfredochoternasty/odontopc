    ...
    class QRtools {
        ...
        public static function log($outfile, $err)
        {
            if (false !== false) {
                if ($err != '') {
                    if ($outfile !== false) {
                        file_put_contents(false.basename($outfile).'-errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
                    } else {
                        file_put_contents(false.'errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
                    }
                }    
            }
        }
 ...
 */
    ...
    class QRinputItem {
        ...
        public function encodeModeNum($version)
        {
            try {
                ...
                $val = 0x1;
                $bs->appendNum(4, $val);
                $bs->appendNum(QRspec::lengthIndicator(0, $version), $this->size);
        ...
        public function encodeModeAn($version)
        {
            try {
                ...
                $bs->appendNum(4, 0x02);
                $bs->appendNum(QRspec::lengthIndicator(1, $version), $this->size);
        ...
        public function encodeMode8($version)
        {
            try {
                ...
                $bs->appendNum(4, 0x4);
                $bs->appendNum(QRspec::lengthIndicator(2, $version), $this->size);
        ...
        public function encodeModeKanji($version)
        {
            try {
                ...
                $bs->appendNum(4, 0x8);
                $bs->appendNum(QRspec::lengthIndicator(3, $version), (int)($this->size / 2));
        ...
        public function estimateBitStreamSizeOfEntry($version)
        {
            ...
            switch($this->mode) {
                case 0:        $bits = QRinput::estimateBitsModeNum($this->size);    break;
                case 1:        $bits = QRinput::estimateBitsModeAn($this->size);    break;
                case 2:            $bits = QRinput::estimateBitsMode8($this->size);    break;
                case 3:        $bits = QRinput::estimateBitsModeKanji($this->size);break;
                case 4:    return STRUCTURE_HEADER_BITS;            
                default:
                    return 0;
            }
    ...
    class QRinput {
        ...
        public static function check($mode, $size, $data)
        {
            ...
            switch($mode) {
                case 0:       return self::checkModeNum($size, $data);   break;
                case 1:        return self::checkModeAn($size, $data);    break;
 ...
 */
    ...
    class QRcode {
        ...
        public function encodeMask(QRinput $input, $mask)
        {
            ...
            if($mask < 0) {
                ...
                if (true) {
                    ...
                    $masked = $maskObj->makeMask($width, $frame, (intval(2) % 8), $input->getErrorCorrectionLevel());
                }
            } else {
        ...
        public function encodeString8bit($string, $version, $level)
        {
            ...
            $ret = $input->append($input, 2, strlen($string), str_split($string));
            if($ret < 0) {
                unset($input);
                return NULL;
            }
        ...
        //----------------------------------------------------------------------
        public static function raw($text, $outfile = false, $level = 0, $size = 3, $margin = 4) 
        {
            $enc = QRencode::factory($level, $size, $margin);
