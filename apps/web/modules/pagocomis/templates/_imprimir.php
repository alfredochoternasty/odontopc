  ...
  <?php 
	...
	<table border="1" cellspacing="0" cellpadding="1" width="100%">
		...
		<tr>
			...
			<th style="background: #CCC;">Dev.</th>
			<th style="background: #CCC;">Comisi&oacute;n</th>
		</tr>
		<?php 
			$total_comis_fact = 0;
			$devueltos = $fila->getDevueltos();
			foreach($fila->getDetalle() as $det): 
				$total_dev = $plata_devuelta = 0;
				foreach ($devueltos as $dev) {
					if ($dev->producto_id == $det->producto_id && $dev->nro_lote == $det->nro_lote)
						$total_dev += $dev->cantidad;
				}
				if ($total_dev != 0) {
					$plata_devuelta = $total_dev * $det->precio;
				}
		?>
			<tr>
				...
				<?php $comision = ($det->sub_total - $plata_devuelta) * ($det->getVentasZona()->grupo_porc_desc/100); ?>
				<td><?php echo empty($total_dev)?'':$total_dev ?></td>
				<td>$ <?php echo $comision ?></td>
				<?php $total_comis_fact += $comision; ?>
			</tr>
		<?php endforeach; ?>
		<tr><td colspan="8" style="text-align:right;">Total Comisi&oacute;n&nbsp;</td><td><b style="font-size:16px;">&nbsp;$ <?php echo $total_comis_fact ?></b></td></tr>
	</table>
	<br>
  <?php endforeach;?>
	<table border="1" cellspacing="0" cellpadding="1" width="100%">
